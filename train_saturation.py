...
# load testing data
simulation_data = os.path.join(data_dir, 'Sw_validation_10_steps.h5')
...
sat_t[sat_t < 0.025] = 0.0
sat_t[sat_t > 0.8]   = 0.8

sat_v[sat_v < 0.025] = 0.0
sat_v[sat_v > 0.8]   = 0.8
...
train_y = sat_t[:, step_index, :, :, :]

test_x = np.concatenate([k_v, kvkh_v, phi_v], axis = -1)
test_y = sat_v[:, step_index, :, :, :]
...
import pickle
with open('HISTORY-sat-mse-10steps-hd500-filter_8_16_32_32.pkl', 'wb') as file_pi:
